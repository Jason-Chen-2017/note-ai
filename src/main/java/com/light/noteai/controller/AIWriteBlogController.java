...
public class AIWriteBlogController {
    ...
    public String writeBlog(@RequestBody Prompt prompt) {
        return QianWenUtil.INSTANCE.WriteBlog(prompt.getPrompt());
    }

    @PostMapping("/complete")
    public String complete(@RequestBody Prompt prompt) {
        return QianWenUtil.INSTANCE.Complete(prompt.getPrompt());
    }

    @PostMapping("/aigc")
    public String aigc(@RequestBody Prompt prompt) {

        String lines = QianWenUtil.INSTANCE.Complete(prompt.getPrompt());
...
//                // 文章内容每行的行首空格处理
//                content = QianWenUtil.INSTANCE.trimHeadSpaces(content);
//                // 被\t,\n错误替换的latex公式修复
//                content = QianWenUtil.INSTANCE.fixLatex(content);
